...
export class SeatingComponent {
  ...
  tableEnterPredicate = (table: Table) => (_drag: CdkDrag<string>): boolean =>
    !this.isTableFull(table);

  seatEnterPredicate = (table: Table, seat: number) => (drag: CdkDrag<string>): boolean => {
    const occupant = table.assignments.find(assignment => assignment.seat === seat);
    if (!occupant || occupant.guestId === drag.data) return true;

    const alreadySeated = this.store.tables().some(candidate =>
      candidate.assignments.some(assignment => assignment.guestId === drag.data));
    return alreadySeated || !this.isTableFull(table);
  };
  ...
  async onTableDrop(event: CdkDragDrop<Table>, table: Table): Promise<void> {
    const guestId = event.item.data as string;
    const currentTable = this.store.tables().find(candidate =>
      candidate.assignments.some(assignment => assignment.guestId === guestId));
    if (currentTable?.id === table.id) return;
    await this.placeGuest(guestId, table.id, null);
  }

  async onSeatDrop(event: CdkDragDrop<Table>, table: Table, seat: number): Promise<void> {
    const guestId = event.item.data as string;
    const current = this.store.tables()
      .flatMap(candidate => candidate.assignments.map(assignment => ({ tableId: candidate.id, ...assignment })))
      .find(assignment => assignment.guestId === guestId);
    if (current?.tableId === table.id && current.seat === seat) return;
